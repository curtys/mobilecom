...
void HipFsmBase::handleAddressChange(){
	...
	if(!isRegistration) {
		recordScalar("UPDATE SENT", simTime());
			...
			else
			{
				...
				if (msg->getKind() == UPDATE && !isUpdating)
				{
					...
					if (addrIsValidated)
					{
						...
						recordScalar("UPDATE 1 recieved, UPDATE 2 sent", simTime());
						delete msg;
					}
					...
					else
					{
						...
						recordScalar("UPDATE 3 recieved", simTime());
						//HIPHeaderMessage *upd = check_and_cast<HIPHeaderMessage *>(msg);
				...
				// Send out third Update message (this node is trying to update its interfaces)
					...
					recordScalar("UPDATE 2 recieved, UPDATE 3 sent", simTime());
