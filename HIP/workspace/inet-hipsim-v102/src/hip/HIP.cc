	...
	if (category==NF_L2_DISSOCIATED || category==NF_L2_ASSOCIATED_OLDAP)
    {
		...
		for (int i=0; i<ift->getNumInterfaces(); i++)
		{
			...
			if(!(ie->isLoopback()) && !(ie->isDown())) {
				if(mapIfaceToConnected.find(ie) == mapIfaceToConnected.end() || mapIfaceToConnected[ie] != ie->isConnected()){
					mapIfaceToConnected[ie] = ie->isConnected();
					if(ie->isConnected() == false) {
						// Iface is down, stored value was up
						// If there is an other usable interface, start update
						for (int j=0; j<ift->getNumInterfaces(); j++){
							InterfaceEntry *ie2 = ift->getInterface(j);
							if(!(ie2->isLoopback()) && !(ie2->isDown()) && mapIfaceToConnected[ie2] == true) {
								handleAddressChange();
								//scheduleAt(simTime() + HIP_CHECK_TIMER, interfaceCheck = new cPacket("HIP_UPDATE_CHECK"));
								return;
							}
						}
					}
					else {
						// Iface becomes connected and it is an old AP, the address will not change
						handleAddressChange();
						return;
					}
				}
...
void HIP::handleRvsRegistration(cMessage *msg)
{
	if (msg->isName("HIP_REGISTER_AT_RVS"))
	{
		...
		if (rvsIPaddress.isUnspecified())
		{
			...
			ctrl->setSrcAddr(ift->getInterface(tempIfId)->ipv6Data()->getPreferredAddress());
			ctrl->setDestAddr(IPAddressResolver().resolve(par("dnsAddress")));
			ctrl->setDestPort(23);
...
void HIP::handleMsgFromTransport(cMessage *msg)
{
	...
	if(!exists && listHITtoTriggerDNS.count(originalHIT) == 0)
	{
		...
		ctrl->setSrcAddr(ift->getInterface(tempId)->ipv6Data()->getPreferredAddress());
		ctrl->setDestAddr(IPAddressResolver().resolve(par("dnsAddress")));
		ctrl->setDestPort(23);
